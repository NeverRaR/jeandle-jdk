...
bool JeandleIntrinsicLowering::lower(vmIntrinsics::ID id, const ciMethod* target) {
  ...
  switch (id) {
    ...
    // arraycopy
    case vmIntrinsics::_equalsB:
      return lower_arrayequals(1);
    case vmIntrinsics::_equalsC:
      return lower_arrayequals(2);
...
// ---- lower_arrayequals ----
bool JeandleIntrinsicLowering::lower_arrayequals(size_t element_byte_size) {
  llvm::LLVMContext& ctx = *_interp->_context;
  llvm::IRBuilder<>& b = _interp->_ir_builder;
  llvm::Module& m = _interp->_module;
  llvm::Function* f = _interp->_llvm_func;
  ...
  llvm::Value* array_base_offset = b.getInt32(arrayOopDesc::base_offset_in_bytes(T_BYTE));
  ...
  llvm::Value* left_byte_length = b.CreateMul(llvm::ConstantInt::get(left_length->getType(), element_byte_size), left_length);
  if (left_byte_length->getType() != b.getInt64Ty()) left_byte_length = b.CreateIntCast(left_length, b.getInt64Ty(), false);


  llvm::FunctionType* mem_cmp_type = llvm::FunctionType::get(b.getInt32Ty(), {c_heap_ptr_ty, c_heap_ptr_ty, b.getInt64Ty()}, false);
  llvm::Function* mem_cmp_fn = llvm::Function::Create(mem_cmp_type, llvm::Function::ExternalLinkage, "memcmp", m);
  llvm::Value* call = b.CreateCall(mem_cmp_fn, {left_array_base, right_array_base, left_byte_length});
  llvm::Value* is_equal = b.CreateICmpEQ(call, 
